...
<title>Boletas — {{ $gs->grado->nombre ?? '' }} {{ $gs->seccion->nombre ?? '' }} {{ $anio }}</title>
<style>
...
.tabla-notas { width: 100%; border-collapse: collapse; margin-top: 12px; table-layout: fixed; }
.tabla-notas th {
...
.tabla-notas td { border: 1px solid #ccc; padding: 4px 3px; font-size: 10pt; vertical-align: middle; word-wrap: break-word; }
...
<div class="toolbar">
    <span>
        Boletas &mdash; {{ $gs->grado->nombre ?? '' }} {{ $gs->seccion->nombre ?? '' }} &mdash; {{ $anio }}
        &nbsp;({{ $boletas->count() }} alumno{{ $boletas->count() !== 1 ? 's' : '' }})
    </span>
    <div class="acciones">
        <button class="btn-print" onclick="window.print()">&#128438; Imprimir todas / Guardar PDF</button>
...
@php
    ...
    $inscripcion = $boleta['inscripcion'];
    ...
    <div class="datos">
        <div class="datos-fila">
            <div class="datos-campo col-8">
                ...
                <span class="val">{{ strtoupper($est->nombre_completo) }}</span>
            </div>
